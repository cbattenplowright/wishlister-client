...
const WishlistProductListContainer = () => {
    ...
    const fetchWishlist = async () => {
        try {
            ...
            console.log(`Fetching wishlist for user credentials:, ${auth.user.email}, ${auth.credentials}`);

            const response = await fetch (
                // `https://wishlister-h2tf.onrender.com/api/wishlists/${auth.user.userAccountId}`,
                `http://localhost:8080/api/wishlist-products/user/${auth.user.userAccountId}/${wishlist.wishlistId}`,
                {
                    method: "GET",
                    headers: {
    ...
    return (
        <div className="wishlist-product-list-container">
            <WishlistProductList wishlistProductItems={wishlistProductItems} wishlistName={"Wishlist no.1"}/>
        </div>
    );
}
